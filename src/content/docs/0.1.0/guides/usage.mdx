...
description: How to use nf-pediatric
...
![nf-pediatric pipeline schema](../../../../assets/0.1.0/nf-pediatric-schema.png)
...
<Steps>
  1. **`-profile segmentation`:**
     ...
     :::caution
     **A valid FreeSurfer license file is required for this profile. Specify the path to your license using [`--fs_license`](/nf-pediatric/0.1.0/guides/parameters#segmentation-options).** If you don't have a valid license, you can register for one [here](https://surfer.nmr.mgh.harvard.edu/registration.html).
     :::
...
<Steps>
  1. **`--input`**: for the path to your BIDS directory
     ...
     For more details on how to organize your input folder, please refer to the [inputs section](/nf-pediatric/0.1.0/guides/inputs).
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/nf-pediatric -r 0.1.0 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/nf-pediatric/0.1.0/guides/parameters) of the documentation.
:::

:::caution[Running the pipeline in offline mode]
If the computer nodes you are planning to use do not have access to the internet, there is a few steps to go through to make sure the containers and templates are available during pipeline execution. Please head over to [running with no internet access section](/nf-pediatric/0.1.0/guides/nointernet).
:::
...
<FileTree>
  ...
  * nf-pediatric-v0.1.0/            # Finished results in specified location (defined with --outdir)
    * multiqc/
...
First, go to the [scilus/nf-pediatric releases page](https://github.com/scilus/nf-pediatric/releases) and find the latest pipeline version - numeric only (eg. `0.1.0`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 0.1.0`. Of course, you can switch to another version by changing the number after the `-r` flag.
