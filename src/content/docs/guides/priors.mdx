...
Each of those median normative curves were approximate using single equations, and implemented into `nf-pediatric`. While this is the default option, users can still specify their own FRF using the parameter [`--frf_manual_frf`](/nf-pediatric/guides/parameters/#frf-options). Similarly, COMMIT priors can be specified with [`--commit_para_diff`](/nf-pediatric/guides/parameters/#commit-options), [`--commit_perp_diff`](/nf-pediatric/guides/parameters/#commit-options), and [`--commit_iso_diff`](/nf-pediatric/guides/parameters/#commit-options).
